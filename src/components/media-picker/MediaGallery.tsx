...
export const MediaGallery = ({
            ...
            {stagedFiles.length > 0 && (
              <div className="space-y-5 max-w-3xl mx-auto w-full">
                ...
                >
                  {isUploading
                    ? "Uploading..."
                    : `Upload ${stagedFiles.length} file${stagedFiles.length > 1 ? "s" : ""} to ${uploadFolder || "Root"}`}
                </Button>
              </div>
            )}
          </div>
  ...
  <Dialog
    ...
    size="md"
    actions={{ 
      secondary: { label: "Cancel", onClick: () => setShowMoveDialog(false) } 
    }}
  >
  ...
  <Dialog
    ...
    size="sm"
    actions={{
      secondary: { label: "Cancel", onClick: () => setShowDeleteDialog(false) },
      primary: { 
        label: "Delete", 
      ...
      <div className="flex items-start gap-4 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/30 rounded-lg">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">
            This action cannot be undone. The {selectedCount === 1 ? "file" : "files"} will be permanently removed from your media library.
          </p>
        </div>
      </div>
    </div>
