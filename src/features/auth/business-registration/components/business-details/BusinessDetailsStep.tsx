...
const BusinessDetailsStep: React.FC<BusinessDetailsStepProps> = ({ data, onChange, errors }) => {
  const navigate = useNavigate();

  const handleGSTChange = (hasGST: boolean) => {
    onChange({ ...data, hasGST: hasGST });
    navigate({ to: ".", search: (prev) => ({ ...prev, tab: hasGST ? "with-gst" : "without-gst" }), replace: true });
  };
  return (
    <StepContainer
    ...
    >
      <RadioGroup
        ...
        onChange={(value) => handleGSTChange(value === "yes")}
        error={errors.hasGST}
        groupClassName="mb-2"
        options={[
          { label: "Yes", value: "yes" },
