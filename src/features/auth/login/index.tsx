...
const Login: React.FC = () => {
  ...
  const otpFailedOnce = useRef(false);
  ...
  useEffect(() => {
    if (step === "INPUT_OTP" && otpRef.current && !otp) {
  ...
  const handleLogin = async (otp: string) => {
    ...
    verifyOtpMutation.mutate(
      { phone: phone, otp: otp },
      {
          ...
          try {
            ...
            if (profileResponse?.data) {
              ...
              if (completeUserData.onboarding?.steps) {
                const businessSteps = completeUserData.onboarding.steps.filter(
                  (step: any) => step.step >= 2 && step.step <= 4
                );
                ...
                const verificationStep = completeUserData.onboarding.steps.find(
                  (step: any) => step.step === 5
                );
        ...
        onError: (error) => {
          otpFailedOnce.current = true;
          const errorMessage =
            (error as { message?: string })?.message || "Invalid credentials or login failed.";
  ...
  const handleOtpComplete = (val: string) => {
    if (!otpFailedOnce.current) handleLogin(val);
  };
  ...
  return (
    <div className="w-full px-5 py-6 sm:px-6 md:px-8 lg:px-10 flex flex-col">
      ...
      <div className="mb-6 shrink-0">
        ...
        <div>
          <h1 className="mb-2 text-base-content text-2xl font-semibold sm:text-4xl">Log In</h1>
        ...
        {step === "INPUT_MOBILE" ? (
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="text-sm sm:text-base text-base-content">
              ...
              <span className="font-semibold text-base-content mx-1">+91 {phone}</span>
              <button
